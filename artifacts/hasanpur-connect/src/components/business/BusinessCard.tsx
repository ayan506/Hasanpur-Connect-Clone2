...
import { Star, MapPin, Phone, MessageCircle, Crown, Flame, CheckCircle } from "lucide-react";
...
function BusinessBadges({ isPremium, isVerified }: { isPremium?: boolean; isVerified?: boolean }) {
  if (isPremium && isVerified) {
    return (
      <Badge className="bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-600 hover:to-yellow-500 text-white border-none flex items-center gap-1 shadow-md">
  ...
  return (
    <Card className="overflow-hidden flex flex-col h-full hover-elevate transition-all duration-300">
      <div className="relative aspect-[16/9] w-full overflow-hidden bg-muted">
        {primaryImage ? (
          <img src={primaryImage} alt={business.name} className="w-full h-full object-cover" />
        ) : (
        ...
        <div className="absolute top-2 left-2 flex flex-col gap-1">
          <BusinessBadges isPremium={business.isPremium} isVerified={business.isVerified} />
        </div>
      ...
      <CardContent className="p-4 flex-grow flex flex-col gap-3">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className="text-xs text-muted-foreground">({business.reviewCount || 0} reviews)</span>
          </div>
        </div>
      ...
      <CardFooter className="p-4 pt-0 gap-2">
        <Link href={`/business/${business.slug}`} className="flex-1">
          <Button variant="outline" className="w-full font-medium">View Details</Button>
        </Link>
        {business.whatsapp && business.isPremium && (
          <Button size="icon" className="bg-[#25D366] hover:bg-[#20BD5A] text-white shrink-0"
